...
import React from 'react';
import { DocumentType } from '@/app/types/data'; // Use shared DocumentType
      ...
      <tbody>
        {filteredDocuments.map((doc, index) => (
          ...
          >
            ...
            <td
              className="p-4 text-gray-600 cursor-pointer hover:underline"
              onClick={() => setSelectedDocument(doc)}
            >
              {typeof doc.date === 'string' && !isNaN(Date.parse(doc.date))
                ? new Date(doc.date).toLocaleDateString('vi-VN')
                : doc.date || 'Không xác định'}
            </td>
            <td
              className="p-4 text-sm text-gray-800 cursor-pointer hover:underline"
              onClick={() => setSelectedDocument(doc)}
